...
import { Restaurant, RestaurantModel } from '../../../db/models/restaurants'
...
export class RestaurantSandbox {
    ...
    public static get Restaurant() { return RestaurantModel;}
   
    public static async getAll(page: PaginationParams, lightweight: boolean = true): Promise<(Partial<Restaurant>)[]> {
    ...
    private static findAllByConditionFields(subObj: {[key: string]: any}, page: PaginationParams, lightweight: boolean = true){
        ...
        RestaurantModel.find(subObj, { name: 1, chef: 1 }): 
     ...
     */
    public static async filter(page: PaginationParams, filter: RestaurantsFilterParams, lightweight: boolean = true): Promise<(Partial<Restaurant>)[]> {
        ...
        const query = RestaurantModel.aggregate(pipes);
        return query.exec();    
    }

    public static async getById(id: string): Promise<Partial<Restaurant>> {
    ...
    public static async patchDeactivateById(id: string): Promise<Partial<Restaurant>> {
        return RestaurantModel.findByIdAndUpdate(id, { isActive: false }, {new: true}).lean().exec();
    }
    public static async patchReactivateById(id: string): Promise<Partial<Restaurant>> {
        return RestaurantModel.findByIdAndUpdate(id, { isActive: true }, {new: true}).lean().exec();
    }
}
